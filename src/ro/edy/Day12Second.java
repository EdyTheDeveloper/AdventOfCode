...
public class Day12Second {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("Result: " + new Day12Second().processInput());
	...
	private String processInput() throws Exception {
		long firstIndex = 0;
		List<String> potsList = new ArrayList<>();
		int generation = 0;
		while (true) {
			generation++;
			input = "...." + input + "....";
			...
			int firstPot = input.indexOf("#");
			firstIndex = firstIndex + firstPot;
			input = input.substring(input.indexOf("#"));
			input = input.substring(0, input.lastIndexOf("#") + 1);

			if (potsList.contains(input)) {
				firstIndex = ((long) (50_000_000_000l - generation)) + firstIndex;
				break;
			} else {
				potsList.add(input);
			}
		...
		for (int i = 0; i < charArray.length; i++) {
			if ((charArray[i] + "").equals("#")) {
				sum = sum.add(new BigInteger(currentIndex + ""));
			}
			currentIndex++;
		}
		return "Result: " + sum;
