...
public class Day4Second {
	...
	public static void main(String[] args) {
		System.out.println("Result: " + new Day4Second().processInput());
	}

	private String processInput() {
		Map<Integer, List<SleepInteval>> sleepIntervalsMap = loadSleepIntervalsMap();

		Integer guardMaxSlept = null;
		int maxSleptTimes = -1;
		int maxSleptMinute = -1;
		for (Entry<Integer, List<SleepInteval>> entry : sleepIntervalsMap.entrySet()) {
			MaxSleepMinute maxSleepMinute = getMaxSleepMinute(entry.getValue());
			if (maxSleepMinute.getTimes() > maxSleptTimes) {
				maxSleptTimes = maxSleepMinute.getTimes();
				maxSleptMinute = maxSleepMinute.getMinute();
				guardMaxSlept = entry.getKey();
			}
		}

		return "The guard " + guardMaxSlept + " slept in minute " + maxSleptMinute + " a number of " + maxSleptTimes + " times. Result is "
				+ (guardMaxSlept * maxSleptMinute);
	}

	private MaxSleepMinute getMaxSleepMinute(List<SleepInteval> sleepIntervals) {
		Map<Integer, Integer> minutesSleepMap = new HashMap<>();
		for (int i = 0; i <= 59; i++) {
			minutesSleepMap.put(i, 0);
		}
		for (SleepInteval sleepInterval : sleepIntervals) {
			for (int i = sleepInterval.getStart(); i < sleepInterval.getEnd(); i++) {
				minutesSleepMap.put(i, minutesSleepMap.get(i) + 1);
			}
		}
		int currentMaxMinuteSlept = -1;
		int currentMaxMinute = -1;
		for (Entry<Integer, Integer> sleepInteval : minutesSleepMap.entrySet()) {
			if (sleepInteval.getValue() > currentMaxMinuteSlept) {
				currentMaxMinuteSlept = sleepInteval.getValue();
				currentMaxMinute = sleepInteval.getKey();
			}
		}

		return new MaxSleepMinute(currentMaxMinute, currentMaxMinuteSlept);
	}

	private Map<Integer, List<SleepInteval>> loadSleepIntervalsMap() {
	...
	class MaxSleepMinute {
		private int minute;
		private int times;

		public MaxSleepMinute(int minute, int times) {
			super();
			this.minute = minute;
			this.times = times;
		}
